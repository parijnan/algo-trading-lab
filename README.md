...
│   ├── backtest.py
│   └── data/
│       ├── nifty_15min.csv         (generated by precompute.py — gitignored)
│       ├── nifty_75min.csv         (generated by precompute.py — gitignored)
│       ├── vix_daily.csv           (generated by precompute.py — gitignored)
│       └── trade_logs/             (generated by backtest.py — gitignored)
├── apollo_production/              # Apollo live execution module
│   ├── README.md
...
│   ├── logger_setup.py             # Dual console+file logging (logs/debug.log)
│   ├── technical_indicators.py     # Copied from apollo_backtest/ for self-contained imports
│   ├── tests/
│   │   └── ws_test.py              # WebSocket layer validation harness
│   ├── data/
│   │   └── .gitkeep                # Runtime data gitignored — state, trades, ST cache
│   └── logs/                       # SDK and debug logs — gitignored, created at runtime
