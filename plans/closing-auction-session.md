...
**Test script ready (2026-08-11)**: `research/cas_smartapi_check/check_cas_indicative_price.py` — logs in once, runs a SnapQuote WebSocket subscription (every tick logged, continuous) and a REST `getMarketData(mode="FULL")` poll (every 20s, per instruction) against `NIFTY_TOKEN`/`SENSEX_TOKEN`, from 15:10 to 15:40, then logs out cleanly. Outputs to `research/cas_smartapi_check/outputs/` (`ws_ticks_<date>.csv`, `rest_polls_<date>.jsonl`). Not run yet — **blocked on the standing no-ad-hoc-AngelOne-during-live constraint, now resolved operationally**: the user moved Iris's own cutoff to 15:17 (commit `093ad3d`) specifically to clear this window, and will give an explicit go-ahead once Iris has logged off for the day (earlier than 15:17 on any day with no open position after 15:00). Run only after that go-ahead, not on a timer.
...
4. Has any SEBI/exchange response emerged regarding the reference-price settlement fix proposed in §5.1? **Partial (§5.6, 2026-08-11, unverified secondhand)**: @Am_Shai reports SEBI is probing the Aug 3/4 CAS trades specifically, with figures matching our own §4.1 data; X's own trending panel independently listed the same story. Not confirmed against a primary SEBI source — find one before treating this as settled.
...
8. **New**: does the "market learned from Nifty's Aug 4 event and priced Sensex's Aug 6 expiry in earlier" read (§4.8) hold up on Sensex's *next* expiry (2026-08-13) — i.e. does IV now climb early for Sensex expiries generally, or was Aug 6 specifically anticipatory because it was the first one? **Nifty's own second expiry (2026-08-11, §4.11) is a live open discrepancy, not yet resolved**: measured auction/constituent data was quiet (matching the recent quiet baseline), but a same-day secondhand report (§5.6) claimed an extreme pre-close IV spike (CE 98/PE 175). Check against the actual option tape once it lands via Wednesday's (2026-08-12) ICICI downloader — confirm or correct one side of this before citing either.
...
12. **New (§10.7, standing instruction 2026-08-10): keep observing whether the Sensex overnight-gap risk recedes over time and whether Sensex's own close starts tracking Nifty's more closely.** Two threads to update as each new CAS-era day lands: (a) does NSE's own auction move (the gap-fuel proxy, §4.1/§10.4) keep trending small, or does it revert to Aug 3-4-sized moves on some future day; (b) does the `ClsPric`/`SttlmPric` gap (§4.6) narrow — i.e. does Sensex's index start reflecting the same information Nifty's does by the close, rather than only at the next open. Both readable from data already flowing (`cas_auction_tracking.csv`; bhavcopy or an intraday-tick pull per §10.4's method for the convergence check). No fixed check-in cadence — update as data lands.
13. **New (§5.5, planned 2026-08-11): check whether SmartAPI exposes live CAS indicative prices for Nifty/Sensex during 15:15-15:30**, the same window the accepted "index-referenced triggers go blind" risk (§2, item 1) covers — prompted by the Angel One terminal showing indicative CAS prices starting 2026-08-11. Library-level check (no login) found no dedicated indicative/auction field in the WebSocket schema — if SmartAPI carries this at all, it'll show as `last_traded_price`/`ltp` actually moving during the freeze, or via depth. **Test script ready**: `research/cas_smartapi_check/check_cas_indicative_price.py` — WebSocket SnapQuote (continuous) + REST `getMarketData(FULL)` polling (20s), 15:10-15:40, Nifty + Sensex. **Blocker resolved operationally**: Iris's cutoff moved to 15:17 (commit `093ad3d`) to clear this window for exactly this kind of test; run only after the user's explicit go-ahead once Iris has logged off for the day.
