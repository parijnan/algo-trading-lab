...
(Trade count above is closed trades only — mult 2.0 also has exactly 1 trade still open at data end.) A routine data-only refresh this time (three additional trading days, 2026-09-12 through 2026-09-14, including a rollover-eve evening session) — mult 2.0 gained 1 closed trade (390→391), consistent with ordinary new price data. Mult 2.5-5.5 rows are frozen at their 2026-09-12 values (not recomputed — see the mult-2.5-exclusion note above) — `sweep_p3.py`'s own summary doesn't compute Calmar for the raw (no-SL/target) series — the qualitative finding from when all 8 multipliers were last compared stands regardless: raw Calmar climbed steadily from 5.5 down to 2.5, then flattened extending one step further to 2.0 rather than continuing to climb — the signature that argues against 2.5 being purely an under-explored grid-edge artifact.
...
**Reading this.** CRUDEOIL offers meaningfully more room to scale the *same* capital-equivalent exposure than CRUDEOILM does — roughly 3x, consistently, across the whole size range and both tails checked — but "more room" isn't "unlimited room": past ~1,000 bbl (~10 CRUDEOIL lots), the same qualitative slippage concerns reappear, just later. Whether switching (or splitting exposure across both contracts) is actually worth it also depends on the strategy's own edge holding up equally well on CRUDEOIL — already cross-validated 2026-09-07, re-validated 2026-09-09 at T1=2.2%, Mult 2.0 column refreshed again 2026-09-15 through 2026-09-14's data under the per-trade drawdown methodology — above, with Calmar running lower there (9.48-9.58 vs. CRUDEOILM's 11.56-15.6). The margin-per-lot side is no longer an open question: the `/3` divisor behind §26's dynamic `MARGIN_PER_UNIT` formula (`entry_price × LOT_SIZE / 3 × 4`) was checked by the user against both instruments' own real margin requirements, not derived from CRUDEOILM alone — see `prometheus_production/README.md`'s §26 and `prometheus_configs.py`'s `MARGIN_CONTRACT_VALUE_DIVISOR` comment.
...
**No-slippage result** (refreshed 2026-09-15 through 2026-09-14's data): 412 trades, Rs 55L → Rs 2.15Cr (+290.7%), max drawdown −16.0%, Calmar 18.16. Units start at 6, dip to a low of 4 during the early-March drawdown, and recover to a peak of 21 — a far more modest range than CRUDEOILM's 40→226, because CRUDEOIL's ~10x-larger per-unit margin means the same rupee P&L moves units far less.

**Slippage-adjusted result** (same feedback-loop mechanics as the CRUDEOILM slippage run — units resized from post-slippage capital every trade, and margin per unit itself computed from the same per-trade formula): final capital Rs 1.73Cr (+215.4%), max drawdown −19.0%, Calmar 11.31, peak units damped from 21 to 18. Coefficient sensitivity (0.5x/1x/2x anchor): Calmar 14.78 → 11.31 → 6.68.
...
**Refreshed 2026-09-15 through 2026-09-14's data** (391 trades; `phase3/risk_of_ruin_p3.py`, fixed seed 20260909 for reproducibility — a routine data-only refresh, not a methodology change like the parity-fix refresh that preceded it). Ruin remains vanishingly rare:

**Result: P(ruin) = 0.00%** — 0 of 20,000 simulated paths met the full definition.
- P(max drawdown > 40% at any point): 1.74% (348/20,000 paths).
- Of those 348, every single one recovered within the 2-year horizon — 38.8% within 1 month, 95.7% within 3 months, 100% within 6 months, 100% within a year. None qualified as a "long recovery."
- P(equity ever negative — literal wipeout): 0.00%.
- Max drawdown distribution: p50 16.2%, p90 27.5%, p95 32.4%, p99 43.6% — even the 99th-percentile bad-luck path is only borderline past the 40% mark, not blown through it.
- Median terminal equity after 2 years: ₹4.04 crore, from a ₹50,00,000 base — reflects the combo's edge (win rate 45.52%, avg win ₹3,382 vs. avg loss ₹1,805 per unit at 1-unit sizing), which is exactly why ruin is this rare in the simulation.
...
### CRUDEOIL companion analysis (2026-09-09, refreshed 2026-09-15 through 2026-09-14's data)

Same method (`phase3_crudeoil/risk_of_ruin_p3.py`), run against CRUDEOIL's own 412-trade `bespoke_trade_summary.csv`. **Sizing scale differs from CRUDEOILM's**: CRUDEOILM's 50 units came from a specific real decision ("user's current capital supports scaling to 50 lots," Position sizing section above); there's no equivalent documented decision for CRUDEOIL. This run uses 5 units (Rs 50,00,000 capital, `MARGIN_PER_UNIT`=Rs 10,00,000) — matching CRUDEOIL's own dynamic-sizing simulation's actual starting scale, not a stated capital-allocation decision. If a different CRUDEOIL scale is actually intended, this is a judgment call worth revisiting.

**Result: P(ruin) = 0.00%** (0 of 20,000 paths):
- P(max drawdown > 40% at any point): 3.74% (747/20,000 paths) — still meaningfully higher than CRUDEOILM's 1.74%, consistent with CRUDEOIL's own lower Calmar (9.48 vs. CRUDEOILM's 15.6) and proportionally deeper drawdowns already found in the cross-validation above.
- Of those 747, all recovered within the 2-year horizon (100%, matching CRUDEOILM) — 30.1% within 1 month, 90.8% within 3 months, 99.9% within 6 months, 100% within a year.
- P(equity ever negative): 0.00%.
- Max drawdown distribution: p50 18.4%, p90 32.1%, p95 37.6%, p99 50.7% — the p95/p99 tail still sits meaningfully past the 40% ruin threshold, unlike CRUDEOILM's (27.5%/32.4%-region tail that stayed borderline). CRUDEOIL's ruin risk remains structurally higher than CRUDEOILM's at this sizing, not just a smaller version of CRUDEOILM's result.
- Median terminal equity: Rs 3.56 crore, from a Rs 50,00,000 base (vs. CRUDEOILM's Rs 4.04 crore at 10x the margin-per-unit — not a like-for-like capital-efficiency comparison, since the unit scales themselves were chosen differently, per the sizing-scale note above).
