...
11. **New (§10.4)**: the `ClsPric` shortfall mechanism was quantified cleanly for Aug 3 only (0.692pp mean per-stock shortfall vs 0.866pp index divergence) — Aug 4/5 couldn't be tested the same way because bhavcopy's close-to-close data can't isolate the auction increment from ordinary continuous-session noise. A real test needs each exchange's ~15:14 pre-auction intraday level (AngelOne, blocked while Iris is live) — pull this once the no-AngelOne-during-live constraint lifts (end of day, or once Iris finishes) and re-run the shortfall calc for as many days as possible to see if the Aug 3 ratio (~0.8) holds as a rate.
...
**Quantified follow-up, 2026-08-07 (answering the user's direct question — "could it be to do with what values are actually being used to calculate Sensex"): yes, checked against official NSE/BSE bhavcopy, not inferred.** §4.6 established the `ClsPric`/`SttlmPric` split from Aug 3-4 data; re-pulled fresh for Aug 3, 4, 5, and 7 to quantify it. `SttlmPric` (BSE's own file) matches NSE's close almost exactly on every day checked — e.g. Aug 7: diffs of −0.003% to +0.006% across the 6 heavyweight stocks, i.e. noise. **This is a control from the same BSE file**, and it's what makes the next number meaningful rather than a cross-exchange artifact.

`ClsPric` — the value Sensex's own muted moves are consistent with using — tells a different story on Aug 3 specifically: every one of the 6 stocks closed short of NSE's move, by a consistent 0.23-1.09 percentage points (mean 0.692pp). Nifty's Aug 3 auction move was +0.8178%; Sensex's was −0.0484% — an index-level divergence of 0.866pp, matching the per-stock shortfall closely (ratio 0.80) on an *unweighted* 6-stock sample against two differently-weighted indices. **This directly answers the "individual stock closes aren't too far off" observation**: they aren't wildly different in absolute terms (each stock's shortfall is under 1.1pp), but the shortfall is small, *systematic*, and same-direction across every constituent tested — and because Sensex's own true move that day was itself small, a consistent ~0.7pp-per-stock shortfall is enough to erase nearly all of it. It doesn't take one stock behaving strangely; it takes all of them being quietly short by a similar amount. Note this doesn't contradict §4.4's "zero trades" finding — a moved `ClsPric` with zero fresh auction trades is exactly what you'd expect if `ClsPric` is just the last continuous-session trade (~15:14), carrying the day's continuous move but missing the auction increment entirely; §4.4 and this measurement are the same story at two different resolutions.

**This does not yet replicate as a rate — Aug 4 and Aug 5 don't reproduce it, and the reason is methodological, not a reason to doubt the mechanism.** Re-running the same shortfall calculation for Aug 4 and Aug 5 gives mean shortfalls of −0.283pp and −0.340pp — wrong sign, and not even consistently signed stock-to-stock. Bhavcopy only gives close-to-close, which bundles the auction increment together with ordinary continuous-session cross-exchange divergence (spread/liquidity differences that have nothing to do with CAS). On Aug 3 — the first CAS day, the largest and most uniform shock in the whole sample — the auction increment was large enough to dominate that noise; on the quieter Aug 4-5 sessions it wasn't separable with this data. **A proper test needs each exchange's own pre-auction (~15:14) intraday level, which bhavcopy doesn't carry** — exactly the kind of pull §4.3/§4.4 did via AngelOne for Aug 3-4, but that's blocked today by the standing no-ad-hoc-AngelOne-while-Iris-is-live rule (§8). Flagged as a follow-up (§9), not resolved here. Treat Aug 3 as one clean, mechanism-consistent, code/data-verified observation — not a proven rate.
