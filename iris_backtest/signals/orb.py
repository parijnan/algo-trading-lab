"""ORB — Opening Range Breakout on 1-min bars."""
import sys
...
def detect(df_1min: pd.DataFrame) -> pd.DataFrame:
    """
    Opening range = high/low of the first ORB_MINUTES bars (09:15 to 09:15+N).
    ...
    for date, day in df_1min.groupby(df_1min.index.date):
        ...
        # Require a full opening range window and at least one post-ORB bar
        if len(orb_bars) < ORB_MINUTES - 1 or post_orb.empty:
            continue
