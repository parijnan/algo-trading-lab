...
def load_precomputed():
    ...
    # Convert trend column — stored as string in CSV
...
def get_option_price(option_df: pd.DataFrame, timestamp: pd.Timestamp,
    ...
    Falls back to last available price before timestamp if exact match not found.
    ...
    # Fallback: last available price before timestamp
...
def select_strike(spot: float, expiry: pd.Timestamp,
    ...
    direction: 'bearish' (sell CE) or 'bullish' (sell PE)
    ...
    # Scan OTM strikes from ATM outward
    ...
    for strike in candidates:
        ...
        if delta <= TARGET_DELTA:
            # First (closest to ATM) strike satisfying delta <= 0.20
...
def check_stop_losses(spot: float, sell_strike: int, direction: str,
    ...
    """
    # 1. Index SL — spot moves beyond sell strike by INDEX_SL_OFFSET points
...
def run_backtest(nifty_15: pd.DataFrame, nifty_75: pd.DataFrame,
    ...
    # ------------------------------------------------------------------
    # Trade state — persists across days (positions can be held overnight)
    # ------------------------------------------------------------------
    in_trade    = False
    direction   = None
    sell_strike = None
    ...
    for day_date in trading_days:
        ...
        # Close any open trade at expiry
        if in_trade and expiry is not None:
            if day_date > expiry.date():
                # Expiry has passed — book at last known LTP
                pl_points = _calc_pl(sell_entry, sell_ltp,
                                     buy_entry, buy_ltp)
        ...
        # Skip non-high-VIX days — but don't close existing trades
        ...
        for idx in range(len(day_15)):
            ...
            # Get most recent 75-min trend at or before this 15-min candle
            prior_75 = nifty_75_indexed[nifty_75_indexed.index <= ts]
            if prior_75.empty:
                continue
            last_75  = prior_75.iloc[-1]
            trend_75 = last_75['trend']

            # Skip until both timeframes have valid Supertrend
            if pd.isna(trend_15) or pd.isna(trend_75):
                continue

            # Next candle info for execution
            ...
            if in_trade:
                ...
                if exit_reason:
                    ...
                    all_trades.append(trade_record)
                    day_trade_log.append(trade_record)
                    _log_exit(trade_record)
                    _save_day_log(day_date, day_trade_log)
                    ...
                    # Fall through to check for immediate re-entry below
            ...
            if not in_trade and flip_15:
                ...
                # Bullish: both timeframes agree bullish → sell PE
                if trend_75 == True and trend_15 == True:
                    entry_direction = 'bullish'
                # Bearish: both timeframes agree bearish → sell CE
                ...
                # Need a next candle to execute on
                if not has_next:
                    continue
                ...
                if sel_strike is None:
                    logger.debug(
                        f"  No valid delta strike at {exec_ts} — skipping")
                    continue
                ...
                # Entry prices at next candle open
                sell_entry_raw = get_option_price(
                    sell_opt_df, exec_ts, 'open')
                buy_entry_raw  = get_option_price(
                    buy_opt_df,  exec_ts, 'open')

                if sell_entry_raw is None or buy_entry_raw is None:
                    logger.debug(
                        f"  Option data missing at {exec_ts} — skipping")
                    continue
...
def _save_day_log(day_date, trade_log: list):
    if not trade_log:
        return
    pd.DataFrame(trade_log).to_csv(
        os.path.join(TRADE_LOGS_DIR, f"{day_date}.csv"), index=False)
...
if __name__ == "__main__":
    ...
    all_trades = run_backtest(nifty_15, nifty_75, vix_daily, contracts_df)
