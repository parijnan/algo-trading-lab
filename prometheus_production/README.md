...
graph TD
    ...
    Resume -- Yes --> ResumeWarn[Rebuild pending_trade_row from state;\n_reconcile_positions() -- netqty vs\nexpected, live only, §25;\nSlack: resumed + reconciliation status]
...
**Stale-PID takeover waits for the old process to actually exit, not a fixed sleep** (§24, 2026-09-11): `main()`'s restart handoff used to send SIGTERM to the old PID then sleep a fixed 3s before assuming it was gone and writing fresh `prometheus.pid`/`prometheus_active.flag` — normally safe, since teardown takes ~1s. A live incident showed the gap: the old process's own shutdown got dragged out past 3s by Slack calls retrying/failing against a workspace-level rate limit, so it was still alive, still mid-`finally`, when the new process had already written its own fresh files. When the old process finally finished and ran its own routine `FLAG_PATH.unlink()`, it deleted the NEW process's flag — whose own main loop read that as an operator-pulled circuit breaker and shut itself down too, leaving nothing running. Fixed with `_wait_for_pid_exit()`: polls `os.kill(pid, 0)` (a liveness probe, no signal sent) every 0.3s for up to `_STALE_PID_WAIT_TIMEOUT_SEC` (30s) — a PID can only vanish from the process table after the process's own `finally` block has fully run, so a confirmed disappearance is a real guarantee, not a guess. If the old process still hasn't exited after 30s, escalates to SIGKILL rather than risk racing it indefinitely.

**Position reconciliation on resume, net quantity only** (§25, 2026-09-11): a resumed `in_trade` state used to just Slack "NOT reconciled against the broker's order book — verify manually if in doubt." with no actual check. `_reconcile_positions()` now calls `self.obj.position()` and compares the broker's `netqty` for `state.token` against an expected signed quantity (`±(open lots) × LOT_SIZE`, counting only lots with `status=='open'` — a booked lot1 with lot2 still open is the routine 2-lot scale-out shape) — same pattern Apollo/Athena/Artemis already use in production. Alert-only on a mismatch (`#error-alerts`), never auto-corrects, and deliberately never compares price — a broker fill carries brokerage/other charges the state file's `entry_price` doesn't, so quantity is the only meaningful cross-check (user-specified). **Skipped entirely in `DRY_RUN`** — no real broker position exists for a paper trade. **One open caveat, not independently confirmed beyond what Apollo/Athena/Artemis already rely on unmodified**: this assumes `netqty` already reflects the current total holding for a position carried forward from a prior day (the standard convention across Indian broker APIs), rather than needing `cfbuyqty`/`cfsellqty` added on top — Prometheus's positional design (§2, multi-day holds are the common case) makes this scenario more central here than for any of the other three strategies, so it's worth specifically watching the first few live restarts with an open position for a false mismatch.

**Dynamic `MARGIN_PER_UNIT` — computed live from LTP, not a frozen constant** (§26, 2026-09-11): actual per-lot margin tracks the underlying's price directly, so the static `MARGIN_PER_UNIT` set when crude was cheaper (~₹25,000/lot → the original 100,000) drifts stale as price rises (~₹30,000/lot now, per the user's own observation). `_calculate_margin_per_unit()` now computes it fresh every call as `LTP × LOT_SIZE / MARGIN_CONTRACT_VALUE_DIVISOR × MARGIN_SIZING_MULTIPLIER` (config constants, 3 and 4 — contract value → conservative per-lot margin approximation → the same ×4 that produced the original 100,000 calibration), sourced from `_get_contract_ltp()` (the existing WS-then-REST helper built for the DPL circuit check) — falling back to the static `MARGIN_PER_UNIT` constant only if a live LTP is unavailable, including a zero/bad print (guarded `if not ltp:`, not `is None`, since a `0.0` would otherwise silently zero out the margin requirement). `_calculate_units()`'s dynamic-sizing branch uses this; `_check_margin_sufficient()` uses it unconditionally (regardless of `DYNAMIC_SIZING`), since real affordability is a fact about current price, not about which sizing mode picked the unit count — see plan §26 for the full reasoning, including the resulting live behavior change (a 1-unit entry now needs ~₹120,000+ margin, up from the static ₹100,000). **Backtest-side dynamic-sizing simulation updated to match, 2026-09-11**: `phase3/dynamic_sizing_sim*.py` and `phase3_crudeoil/dynamic_sizing_sim*.py` now compute margin per unit per-trade from that trade's own `entry_price` too, same formula — see `prometheus_backtest/README.md`'s dynamic-sizing sections and root `README.md`'s Prometheus section for the refreshed headline numbers, and both "Dynamic Sizing Simulation" artifacts, rebuilt.
...
**Session report format** (`_send_session_report()`, `#tradebot-updates`). Every P&L figure is **per unit** (Rs/unit, with each trade's actual unit count shown next to it) — the report tracks the strategy's own performance, not the capital-scaled outcome of whatever sizing was live (2026-09-22). One block per trade closed that day showing the **full trade's** P&L with its real entry/exit dates and times (a carryover trade shows its whole life, including a lot1 booked in an earlier session), plus an "Open Position" block if one is still running (its Realised/Unrealised/total). The bottom **Session Total** (2026-09-24) has no combined total — it's split into **Realized P&L** and **Unrealized P&L**. Realized counts only lots whose exit was **booked on that report's own date**: for a carryover trade whose lot1 hit target1 in an earlier session, only lot2's P&L lands in today's Realized, since lot1's was already reported as that earlier session's Realized (a lot that books today on a still-open trade counts too). Unrealized is the mark-to-market of whatever is still open at the last known LTP, summed per unit. Realized and Unrealized are each an unweighted sum of per-unit figures, independent of how many units any one trade carried. The report is also sent by a mid-day Kill Switch teardown (e.g. a restart), in which case Realized is "booked so far today" and the end-of-day report supersedes it.
...
| `TRADE_UPDATE_SEC` | 20 | Slack update cadence while in-trade — matches Artemis's/Athena's convention, not Iris's 10s. Briefly raised to 60s on 2026-09-11 after contributing to the workspace hitting Slack's own `message_limit_exceeded` cap; reverted the same day once the Slack plan was upgraded (§24) |
...
See [`prometheus_backtest/README.md`](../prometheus_backtest/README.md) for the full calibration journey, both phases. **This production module now runs Phase 3's mult-2.0 candidate** (`prometheus_backtest/phase3/`), decided 2026-09-04 — `ST_MULTIPLIER=2.0`, `SL_PCT=2.2`, `TARGET1_PCT=2.2`, `TARGET2_FLAT_PCT=5.0`, replacing the Phase 2 config this table used to show. `TARGET1_PCT` changed from 2.0 to 2.2 on 2026-09-09 (see `prometheus_backtest/README.md`'s Phase 3 caveat #1) — this is Prometheus's final exit configuration, live in production since 2026-09-14 (`DRY_RUN=False`, `plans/prometheus-live-cutover.md`); table below recomputed 2026-09-19 under a corrected contract-rollover loader (`prometheus_backtest/data_loader_p3.py`, shared with CRUDEOIL) — the prior same-day routine data refresh turned out to have been pricing part of the dataset off the wrong, soon-to-expire contract near every historical rollover; see `prometheus_backtest/README.md`'s "Contract-rollover parity fix" note for the full finding.
...
| Trades | 418 (corrected 2026-09-19, refreshed 2026-09-22 through 2026-09-21's data) | 226 (refreshed 2026-09-04, through 2026-09-03) |
| Win rate | 44.26% | 55.8% |
| Total P&L | ₹207,995 | ₹42,778 |
| Max drawdown | −₹16,831 (per-trade series) | −₹14,943 |
| Calmar | 12.36 (per-trade, methodology-comparable to the mult-2.5 candidate) | 2.86 (unitless) / 4.84 (annualized, ₹1L capital basis) |
| Cross-validation | Cross-validated on CRUDEOIL 2026-09-07, re-validated 2026-09-09 at T1=2.2%, corrected 2026-09-19 under the same contract-rollover fix as CRUDEOILM — Calmar 10.18, edge held (see `prometheus_backtest/README.md`'s Phase 3 caveat #3) — **both instruments now share the same corrected loader**, so this is a genuine like-for-like comparison | Confirmed on CRUDEOIL (full-size contract) before being trusted |
