...
13. **New (§5.5, planned 2026-08-11): check whether SmartAPI exposes live CAS indicative prices for Nifty/Sensex during 15:15-15:30**, the same window the accepted "index-referenced triggers go blind" risk (§2, item 1) covers — prompted by the Angel One terminal showing indicative CAS prices starting 2026-08-11. **Plan: test both WebSocket (existing `SmartWebSocketV2`/`websocket_feed.py` subscription) and REST polling at 20-second intervals**, on the next live CAS window, to see whether either channel updates during the freeze rather than holding the last 15:14/15:15 tick. **Standing constraint to resolve before running this live**: `feedback_no_angelone_during_live` — no ad-hoc AngelOne logins while a live strategy (Iris) is trading, since a fresh `generateSession()` under the same credentials may share rate limits with (or, unconfirmed, could invalidate) Iris's own live session token. Iris is currently force-routed and likely live on any given weekday — resolve how to run this test safely (separate API key, piggyback on the existing production WS connection instead of a new login, or explicit one-off exception) before attempting it, don't just run it.
