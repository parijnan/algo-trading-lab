...
# ── MCX DPL (Daily Price Limit) circuit-breaker detection (plan §11a) ────────
# 2026-09-11: replaced the original chain-heuristic detector (2+ consecutive
# flat 1-min runs, inferred from OHLC patterns) with a direct read of the
# exchange's own live circuit limits. Confirmed live (logged in and checked
# both REST getMarketData(mode='FULL') and WS SNAP_QUOTE mode against real
# CRUDEOILM data, 2026-09-11 11:13 IST): AngelOne's SmartAPI publishes
# 'upperCircuit'/'lowerCircuit' (REST) / 'upper_circuit_limit'/
# 'lower_circuit_limit' (WS, paise-scaled) directly — both agreed exactly
# (UC=10108.0, LC=9332.0), and cross-checked against the DPL formula
# (previous close x 1.04/x0.96 = 10108.8/9331.2) to within 0.01%. Reading
# the field directly is strictly better than computing it ourselves: same
# broker call either way, no reference-price reconstruction needed (local
# 1-min history can be truncated on some days — confirmed for pre-DST-
# changeover dates), self-updates through every relaxation step
# automatically, and never goes stale if MCX ever changes the percentage
# slabs. The old chain heuristic (run-length/volume/gap calibration) is
# gone — this is strictly better on every axis it covered, plus it catches
# an isolated single-step freeze (e.g. 2026-09-10 21:18), which the old
# heuristic explicitly could not.
DPL_CIRCUIT_POLL_ENABLED = True   # kill switch — zero effect on trading either way, alert-only
...
# ── Slack ─────────────────────────────────────────────────────────────────────
TRADE_UPDATE_SEC = 20      # briefly raised to 60s on 2026-09-11 after this cadence contributed
                            # to the workspace hitting Slack's own message_limit_exceeded cap
                            # that day (see plans/prometheus-phase3-production.md §24) -- reverted
                            # back to 20s the same day once the Slack plan was upgraded, removing
                            # the message-cap constraint. Matches Artemis's/Athena's convention,
                            # not Iris's 10s.
