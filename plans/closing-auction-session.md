...
**Point 2 — "index_sl is blind because the index freezes at 15:15."** True, but narrower than the full picture: Artemis has **no overnight SL evaluation of any kind**, independent of CAS. `leto.py` is a single daily cron entry (`15 9 * * 1-5`) that calls `generateSession` and `terminateSession` exactly once per day (per `README.md`, confirmed architecturally — no persistent overnight process exists; `slack_listener.service` is the only always-on daemon, and it handles Slack commands, not price monitoring). Between one day's teardown and the next day's 9:15 launch, `index_sl` and `option_sl` are not evaluated at all — this has always been true, pre-CAS included, and the Aug 5 incident record itself confirms it (breach *detected at market open*, not when it actually occurred overnight). The CAS-specific 15:15-15:40 freeze (accepted-risk §2.1) is a second, narrower blind window layered on top of this pre-existing one, not the root cause of the overnight gap-through by itself.
