...
- **`⚙️ Manage Sizing`**: Opens a modal for surgical position sizing updates — toggle between Dynamic Auto-Sizing and Fixed Lots, and set the lot count (units, for Prometheus) for Artemis, Athena, Iris, or Prometheus. Updates are written to a gitignored `data/sizing_override.json` inside each strategy directory, overriding the defaults in its own `<strategy>_configs.py`. Artemis/Athena/Iris read this at process start only — a change takes effect on that strategy's next restart. **Prometheus reads it live** (2026-09-07, `prometheus_functions.resolve_live_sizing()`) — a change takes effect on its very next entry (fresh or a Rule 7 flip), no restart needed, since Prometheus is near-permanently in-trade unlike the other three.
- **`🧹 Clear Sizing Override`**: Opens a modal to delete a strategy's `sizing_override.json`, reverting it to whatever its own `<strategy>_configs.py` currently hardcodes. For when a Slack override (set e.g. while away from a laptop) has since been pushed permanently into that file's own config and the override needs to stop shadowing it. Same live/next-restart split as above by strategy.
...
| Sizing | "Units" (1 unit = 2 lots) — static (go-live: 1 unit; 5 units since 2026-09-21; margin/unit computed live from LTP) or dynamic (Artemis's formula), Slack-switchable |
...
| Status | **Live — `DRY_RUN=False`, since 2026-09-15 (`STATIC_UNITS=1` at go-live, bumped to 5 on 2026-09-21)** (`prometheus-live-cutover.md`), after a clean two-week DRY_RUN pass on Delos (mult-2.0 config, ST_15 chart-validated by the user, trades #9–#20+ with no recurrence of the 2026-08-31 fill-confirmation incident). Phase 3 build complete as of 2026-09-04, including contract-rollover-under-an-open-position (§3–§9 of the Phase 3 plan: the `state.token` invariant, the evening trigger, missed-rollover recovery, the full prefetch/veto/flatten/reopen timeline, Rule 7's combined order with a stuck-partial-fill retry, historical-basis SL/target recalibration, and the two-linked-rows trade-log schema) — **exercised by a real rollover for the first time 2026-09-14** (see above), which surfaced and fixed a genuine gap (the mid-day flat→new-contract switch only ran once, at setup). The 1h/15m entry filter (§17) is fully wired, unit-tested, and gated off (`ENTRY_FILTER_1H_ALIGN_ENABLED=False`) — backtested 2026-09-04 (Phase 4), no grid cell beat the unfiltered baseline, shelved. The §12a provisional-boundary computation (built 2026-09-04) is **enabled** (`PROVISIONAL_BOUNDARY_ENABLED=True`) — stress-tested live under DRY_RUN (a provisional flip confirmed correct by the real candle a minute later), then deliberately kept on for the live cutover rather than reverted to shadow-only. Order-update WebSocket verified live for MCX — the first genuinely live order (2026-09-15, entry `orderid=260915000154671`) resolved via WS as expected. |
...
|---|---|---|
| Backtest (Mult 2.0 corrected 2026-09-19 for a contract-rollover bug in `data_loader.py`'s front-month selection — see `prometheus_backtest/README.md`'s "Contract-rollover parity fix"; Mult 2.5 frozen at its 2026-09-12 values, not rollover-fixed) | 424 trades · WR 44.3% · ₹209,401 total P&L · Calmar 12.44 | 296 trades · WR 48.7% · ₹138,466 total P&L · Calmar 11.56 |

Mult 2.0 leads on Calmar (12.44 vs 11.56, a thin margin now — see `prometheus_backtest/README.md`'s "Contract-rollover parity fix" for the two-layer methodology correction behind this number) — the mult-2.0 decision stands, and T1=2.2% was independently re-confirmed as the fine-grid Calmar-optimal choice under the corrected simulator (14.76, the single highest value in that re-run grid at its own vintage), not just a plateau pick as the original finding had it.
...
**Dynamic-sizing equity simulation** (`phase3/dynamic_sizing_sim.py`, 2026-09-08, sizing formula updated 2026-09-11, corrected 2026-09-19 for the contract-rollover fix — see `prometheus_production/README.md`'s §26 and `prometheus_backtest/README.md`'s own writeup below): what if Prometheus had gone live on 2026-01-30 with ₹50,00,000 and `DYNAMIC_SIZING=True` the whole way, letting units compound with realised P&L exactly as `_calculate_units()` would live. Margin per unit is no longer a frozen ₹1,00,000 — it's recomputed at every entry from that trade's own price (`entry_price × LOT_SIZE / 3 × 4`, mirroring `Prometheus._calculate_margin_per_unit()`), ranging ₹74,413–₹1,43,107 over the backtest window (unchanged by the rollover fix — a price-level range, not a date-coverage one). Result: 424 trades, ₹50L → ₹2.45Cr (+390.0%), max drawdown −16.1%, Calmar 24.19 — units run 36 to 220 (starting at 63, not 50, since early-2026 crude priced margin lower than the old frozen constant implied), with zero slippage modeled. Read as how the sizing mechanics compound, not a realistic forecast at that scale. [Chart + table](https://claude.ai/code/artifact/ca487422-3376-46a4-8237-6249ec779162); detailed CSVs in `phase3/data_sweep/mult_2.0/` (gitignored, run the script to regenerate).

**Slippage-adjusted extension** (`phase3/dynamic_sizing_sim_slippage.py`, 2026-09-08, corrected 2026-09-19 for the contract-rollover fix): same 424 trades, but each fill's slippage is now `0.3·√(participation_%)` ticks — participation measured against real CRUDEOILM 1-min volume at that fill's own timestamp, coefficient anchored to the liquidity analysis's own stated number (25% participation → 1.5 ticks) — and, critically, fed back into capital before the next trade's units are sized, so a worse fill this trade damps how big the next one gets. Result: peak units drops from 220 to 150, final capital ₹1.52Cr (+204.4%) vs. the no-slippage ₹2.45Cr, Calmar 8.31 vs. 24.19. Ranking holds across a 0.5×–2× coefficient sensitivity sweep (Calmar 14.10 → 8.31 → 3.21). Same [artifact](https://claude.ai/code/artifact/ca487422-3376-46a4-8237-6249ec779162), appended below the no-slippage results; CSVs alongside the originals in `phase3/data_sweep/mult_2.0/` as `dynamic_sizing_trades_slippage.csv` / `dynamic_sizing_equity_curve_slippage.csv`.

**Same simulation on CRUDEOIL, the main contract** (`phase3_crudeoil/dynamic_sizing_sim.py` + `dynamic_sizing_sim_slippage.py`, 2026-09-08, sizing formula updated 2026-09-11, corrected 2026-09-19 under the same contract-rollover fix as CRUDEOILM): same question, asked of CRUDEOIL instead of the mini, starting capital ₹55,00,000 (user-supplied). Margin per unit now ranges ₹7,43,867–₹14,30,667 (`entry_price × LOT_SIZE(100) / 3 × 4`), no longer the flat ₹10,00,000 the old constant used. No-slippage: 437 trades, ₹55L → ₹2.42Cr (+340.1%), max drawdown −17.0%, Calmar 20.00, units 3→21 (starting at 6) — a far more modest range than CRUDEOILM's 36→220, since CRUDEOIL's ~10x-larger per-unit margin means the same rupee P&L moves units far less. Slippage-adjusted (same model, same 0.3·√(participation_%) anchor, carried over from the CRUDEOIL liquidity comparison rather than re-fit): final capital ₹1.93Cr (+251.1%), Calmar 13.03, peak units damped from 21 to 17. [Chart + table](https://claude.ai/code/artifact/704b21e1-1343-489b-8793-7d19240279ef) (separate artifact, same structure as the CRUDEOILM one); CSVs in `phase3_crudeoil/data_sweep/mult_2.0/` (gitignored).
