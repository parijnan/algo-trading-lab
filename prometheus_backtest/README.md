...
**No-slippage result**: 409 trades, Rs 55L → Rs 2.14Cr (+289.5%), max drawdown −16.0%, Calmar 18.08. Units start at 6, dip to a low of 4 during the early-March drawdown, and recover to a peak of 21 — a far more modest range than CRUDEOILM's 40→226, because CRUDEOIL's ~10x-larger per-unit margin means the same rupee P&L moves units far less.

**Slippage-adjusted result** (same feedback-loop mechanics as the CRUDEOILM slippage run — units resized from post-slippage capital every trade, and margin per unit itself computed from the same per-trade formula): final capital Rs 1.74Cr (+215.9%), max drawdown −19.0%, Calmar 11.34, peak units damped from 21 to 18. Coefficient sensitivity (0.5x/1x/2x anchor): Calmar 14.73 → 11.34 → 6.66.
