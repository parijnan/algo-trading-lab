...
def scan_strikes_for_premium(
    ...
    Mirrors live Artemis credit_spread.py initialize_spread() exactly:
    scans all strikes within max_steps and returns the one whose LTP is
    closest to target_premium by absolute difference — either side.
    For example with target Rs 120: Rs 125 is chosen over Rs 110 since
    |125-120| = 5 < |110-120| = 10.

    Scanning stops early once prices drop to near zero (no point continuing
    further OTM). max_steps is a safety cap.
    Returns (strike, close_price) or (None, None) if no data found.
    """
    best_strike = None
    best_price  = None
    best_diff   = float('inf')
    ...
    for step in range(max_steps):
        ...
        if diff < best_diff:
            ...
            best_strike = strike
            best_price  = price
        # Once price has dropped well below the target and is moving further
        # away, no closer strike will be found further OTM — stop scanning.
        if best_price is not None and price < target_premium and diff > best_diff:
            break
