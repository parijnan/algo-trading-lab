...
4. **If the terminal print lands before 15:29, synthesize additional flat candles (at the print's value) for every minute from the transition through 15:29 inclusive.** This is not optional/automatic — confirmed by checking the actual raw data, not assumed: on 2026-08-06 the terminal print was at 15:28 and the raw API has *no row at all* for 15:29 (verified: the saved CSV has exactly 17 rows, last one at 15:28). The chart still shows a flat 15:29 candle. Since Iris's live reconstruction has no equivalent to the data pipeline's `extend_to_day_close` to fall back on, this step has to be explicit here — it's the one piece of §5's chart-matching behavior that doesn't come from the raw feed under any circumstances and has to be manufactured.
5. The reconstructed series **stops at 15:29** — no flat-extension out to 15:39. That's a data-pipeline-only construct (§9) for a different purpose (row-count alignment with the extended derivatives session) and must not leak into what Iris resamples or computes ST on, per §5's chart-parity rule.
