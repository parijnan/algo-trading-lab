...
**Raw signal-quality sweep results** (`ST_PERIOD=10`, no SL/target/EOD, mult 2.0 refreshed 2026-09-16 through 2026-09-15 — part of the routine `refresh_pipeline.py` chain, see "Routine backtest refresh" below; mult 2.5-5.5 rows are frozen at their 2026-09-12 values per the mult-2.5-exclusion decision (2026-09-12, code-level since 2026-09-13) — `sweep_p3.py`'s own `ST_MULTIPLIER_GRID` only computes mult 2.0 now):

| Multiplier | Trades | Win % | Total P&L | Max DD | Calmar |
|---|---|---|---|---|---|
| 2.0 | 394 | 42.6% | ₹159,410 | (see sweep_p3_summary.csv) | — |
...
(Trade count above is closed trades only — mult 2.0 also has exactly 1 trade still open at data end.) A routine data-only refresh this time (one additional trading day, 2026-09-15) — mult 2.0 gained 3 closed trades (391→394), consistent with ordinary new price data. Mult 2.5-5.5 rows are frozen at their 2026-09-12 values (not recomputed — see the mult-2.5-exclusion note above) — `sweep_p3.py`'s own summary doesn't compute Calmar for the raw (no-SL/target) series — the qualitative finding from when all 8 multipliers were last compared stands regardless: raw Calmar climbed steadily from 5.5 down to 2.5, then flattened extending one step further to 2.0 rather than continuing to climb — the signature that argues against 2.5 being purely an under-explored grid-edge artifact.
...
2. **The two candidates are structurally different strategies, not the same mechanism at different scale.** Exit-reason mix (lot1 / lot2, of trades reaching each outcome; Mult 2.0 columns refreshed 2026-09-16 through 2026-09-15's data, Mult 2.5 columns frozen at their 2026-09-12 values per the mult-2.5-exclusion decision):

   | | Mult 2.0 lot 1 | Mult 2.0 lot 2 | Mult 2.5 lot 1 | Mult 2.5 lot 2 |
   |---|---|---|---|---|
   | trend_flip | 225 (57.1%) | 314 (79.7%) | 45 (15.2%) | 118 (39.9%) |
   | target | 140 (35.5%) | 47 (11.9%) | 145 (49.0%) | 52 (17.6%) |
   | stop_loss | 29 (7.4%) | 33 (8.4%) | 106 (35.8%) | 126 (42.6%) |

At 2.5, the tight 1.0% SL does most of the work (largest single exit-reason bucket for both lots). At 2.0, the wide 2.2% SL barely intervenes — most trades just ride to the raw trend_flip exit. **That trend_flip bucket is not benign for mult 2.0's lot 1**: 225 trades, only 19.1% win rate, −₹120,360 in aggregate — the single biggest loss center in the whole 2.0 system, bigger than the SL bucket itself (−₹60,823). The SL is correctly sized to catch *extreme* individual losers (mean −₹2,097/trade vs. trend_flip's −₹535), but the real drag on 2.0's lot 1 is a large population of trades that never reach either target and bleed out slowly — a signal-quality issue, not something a different SL or T1 fixes. Lot 2's trend_flip, by contrast, is genuinely closer to breakeven (−₹95 avg, 38.2% win rate) — the "let it play out" framing holds there, just not for lot 1. (This refresh's 3 newly-closed trades — Sep 15's new sessions — all exited both lots via trend_flip: lot1 trend_flip 222→225 and lot2 trend_flip 311→314, both +3, while every target/SL cell for mult 2.0 is unchanged from the prior refresh.)
3. **CRUDEOIL cross-validation — done 2026-09-07, re-run 2026-09-09 at T1=2.2%, Mult 2.0 column refreshed again 2026-09-16 through 2026-09-15's data under the per-trade drawdown methodology** (see the table above): the edge replicates directionally on the full-size contract, but Calmar drops for both candidates (15.58→9.46 for mult 2.0, 11.56→9.58 for mult 2.5) — CRUDEOIL's drawdowns run proportionally deeper than CRUDEOILM's, not just larger by the 10x lot-size ratio. Doesn't change the mult-2.0 production decision (CRUDEOILM is the live-traded instrument, where mult 2.0 leads clearly), but means the live strategy's risk profile shouldn't be assumed to carry over unchanged if ever run on CRUDEOIL instead.
4. **No transaction costs modeled** (same convention as v1/Phase 2) — mult 2.0 has the highest trade count of any candidate (394 vs. 2.5's 296), making it the most cost-exposed once slippage/brokerage are added.
...
### Historical ATM-straddle implied volatility (built 2026-09-17)

Folder: `prometheus_backtest/phase3_fyers/options_iv/`. Built to test a theory: does the *level* of implied volatility the market is pricing in predict Prometheus's trade outcomes? (A related theory — does raw *entry price level* predict returns — was tested first and rejected: Spearman rho≈0.02, p≈0.78, no correlation in either regime.)

**Methodology**: for every 15-min boundary (matching ST_15, the strategy's own signal timeframe) in CRUDEOILM's continuous front-month price series — the same series, same rollover logic, that produced `data_sweep/mult_2.0/bespoke_trade_summary.csv` — resolve the paired options expiry cycle (options expire 2-4 days before the futures contract they settle into, confirmed directly against Fyers), pick the *real* listed strike nearest to that boundary's own futures price (not an assumed fixed spacing), and back out implied volatility for both the call and put leg via **Black-76** (options-on-futures use Black-76, not vanilla Black-Scholes, since the underlying is already a forward price with no separate dividend yield — confirmed against Zerodha Varsity and corroborated by Angel One's own `OPTFUT` instrument classification and Fyers's option-chain schema, which returns a distinct `fp` "future price" field). No new pricing library needed — `mibian.Me` (Merton model, already used elsewhere in this repo for Greeks) reproduces Black-76 exactly when `dividendYield` is set equal to the risk-free rate and the futures price is passed in place of spot; verified against `mibian`'s own source before relying on it.

Resampling reuses the DST-aware bucketing principle from `prometheus_production/prometheus_functions.py`'s own `_resample_1m_to_Nmin` (a DST-shortened session's trailing partial bucket, e.g. 23:45-23:55, is always kept as one candle, never dropped or split) — but as its own adapted copy (`options_iv/resample_utils.py`), not a reuse of that exact function, because the production version resolves `CLOSING_TIME` once at import time from *today's* real date, which is correct for live trading but silently wrong for a multi-year historical backfill spanning many DST transitions (confirmed directly: it dropped a real day's own trailing bucket in testing). The adapted version re-resolves the closing time per historical day instead.

**Coverage, all confirmed empirically, not assumed**: options history on Fyers only goes back to 2024-06-14 (14 earlier futures cycles, 2023-04 through 2024-05, have no options data behind them at all); a further, previously-unknown full-cycle gap exists for the 2025-10-16 options expiry; and the already-documented 2026-03-13→2026-06-29 Fyers futures gap (above) has a matching options-side gap. A days-to-expiry floor (`MIN_DAYS_TO_EXPIRY=1.0`) also excludes each option's own final trading day — Black-76 IV is numerically unstable as time-to-expiry approaches zero (a well-known artifact, not a bug: confirmed directly, every boundary with `atm_iv>250%` had under ~5 hours to expiry) — which costs nothing analytically since Prometheus itself never holds a position that close to an option's own expiry (rollover happens well before). Net: **1,134 of 2,476 mult-2.0 trades (45.8%) get a usable IV-at-entry value** — a real ceiling on coverage, not a full-history series, but a solid sample concentrated toward the back half of the trade history (the "good regime" window prior research already cares about most).

One real market event surfaced directly in the data, not filtered out: a sustained IV spike across the entire 2026-03-09 session (up to ~250% on a 7-day-to-expiry ATM straddle) — attributed to the Strait of Hormuz crisis, which peaked that month; kept as genuine signal, not treated as a data artifact.

Output: `options_iv/data/atm_iv_15min.csv` (23,338 rows; `futures_price`, `atm_strike`, `ce_price`/`pe_price`, `ce_iv`/`pe_iv`, `atm_iv`) — gitignored like every other generated `data/` directory in this tree; re-run the five numbered pipeline scripts in `options_iv/` to regenerate. The IV-vs-trade-outcome correlation analysis itself has not been run yet — this section covers the data build only.
...
**Reading this.** CRUDEOIL offers meaningfully more room to scale the *same* capital-equivalent exposure than CRUDEOILM does — roughly 3x, consistently, across the whole size range and both tails checked — but "more room" isn't "unlimited room": past ~1,000 bbl (~10 CRUDEOIL lots), the same qualitative slippage concerns reappear, just later. Whether switching (or splitting exposure across both contracts) is actually worth it also depends on the strategy's own edge holding up equally well on CRUDEOIL — already cross-validated 2026-09-07, re-validated 2026-09-09 at T1=2.2%, Mult 2.0 column refreshed again 2026-09-16 through 2026-09-15's data under the per-trade drawdown methodology — above, with Calmar running lower there (9.46-9.58 vs. CRUDEOILM's 11.56-15.58). The margin-per-lot side is no longer an open question: the `/3` divisor behind §26's dynamic `MARGIN_PER_UNIT` formula (`entry_price × LOT_SIZE / 3 × 4`) was checked by the user against both instruments' own real margin requirements, not derived from CRUDEOILM alone — see `prometheus_production/README.md`'s §26 and `prometheus_configs.py`'s `MARGIN_CONTRACT_VALUE_DIVISOR` comment.
...
**No-slippage result** (refreshed 2026-09-16 through 2026-09-15's data): 415 trades, Rs 55L → Rs 2.14Cr (+289.0%), max drawdown −16.0%, Calmar 18.06. Units start at 6, dip to a low of 4 during the early-March drawdown, and recover to a peak of 21 — a far more modest range than CRUDEOILM's 40→226, because CRUDEOIL's ~10x-larger per-unit margin means the same rupee P&L moves units far less.

**Slippage-adjusted result** (same feedback-loop mechanics as the CRUDEOILM slippage run — units resized from post-slippage capital every trade, and margin per unit itself computed from the same per-trade formula): final capital Rs 1.73Cr (+213.7%), max drawdown −19.0%, Calmar 11.22, peak units damped from 21 to 18. Coefficient sensitivity (0.5x/1x/2x anchor): Calmar 14.69 → 11.22 → 6.62.
...
**Refreshed 2026-09-16 through 2026-09-15's data** (394 trades; `phase3/risk_of_ruin_p3.py`, fixed seed 20260909 for reproducibility — a routine data-only refresh, not a methodology change like the parity-fix refresh that preceded it). Ruin remains vanishingly rare:

**Result: P(ruin) = 0.00%** — 0 of 20,000 simulated paths met the full definition.
- P(max drawdown > 40% at any point): 1.71% (343/20,000 paths).
- Of those 343, every single one recovered within the 2-year horizon — 34.1% within 1 month, 93.0% within 3 months, 100% within 6 months, 100% within a year. None qualified as a "long recovery."
- P(equity ever negative — literal wipeout): 0.00%.
- Max drawdown distribution: p50 16.1%, p90 27.6%, p95 32.6%, p99 44.3% — even the 99th-percentile bad-luck path is only borderline past the 40% mark, not blown through it.
- Median terminal equity after 2 years: ₹3.97 crore, from a ₹50,00,000 base — reflects the combo's edge (win rate 45.69%, avg win ₹3,348 vs. avg loss ₹1,801 per unit at 1-unit sizing), which is exactly why ruin is this rare in the simulation.
...
### CRUDEOIL companion analysis (2026-09-09, refreshed 2026-09-16 through 2026-09-15's data)

Same method (`phase3_crudeoil/risk_of_ruin_p3.py`), run against CRUDEOIL's own 415-trade `bespoke_trade_summary.csv`. **Sizing scale differs from CRUDEOILM's**: CRUDEOILM's 50 units came from a specific real decision ("user's current capital supports scaling to 50 lots," Position sizing section above); there's no equivalent documented decision for CRUDEOIL. This run uses 5 units (Rs 50,00,000 capital, `MARGIN_PER_UNIT`=Rs 10,00,000) — matching CRUDEOIL's own dynamic-sizing simulation's actual starting scale, not a stated capital-allocation decision. If a different CRUDEOIL scale is actually intended, this is a judgment call worth revisiting.

**Result: P(ruin) = 0.00%** (0 of 20,000 paths):
- P(max drawdown > 40% at any point): 3.48% (695/20,000 paths) — still meaningfully higher than CRUDEOILM's 1.71%, consistent with CRUDEOIL's own lower Calmar (9.46 vs. CRUDEOILM's 15.58) and proportionally deeper drawdowns already found in the cross-validation above.
- Of those 695, all recovered within the 2-year horizon (100%, matching CRUDEOILM) — 30.1% within 1 month, 89.9% within 3 months, 99.7% within 6 months, 100% within a year.
- P(equity ever negative): 0.01%.
- Max drawdown distribution: p50 18.6%, p90 31.7%, p95 37.1%, p99 49.8% — the p95/p99 tail still sits meaningfully past the 40% ruin threshold, unlike CRUDEOILM's (27.6%/32.6%-region tail that stayed borderline). CRUDEOIL's ruin risk remains structurally higher than CRUDEOILM's at this sizing, not just a smaller version of CRUDEOILM's result.
- Median terminal equity: Rs 3.53 crore, from a Rs 50,00,000 base (vs. CRUDEOILM's Rs 3.97 crore at 10x the margin-per-unit — not a like-for-like capital-efficiency comparison, since the unit scales themselves were chosen differently, per the sizing-scale note above).
