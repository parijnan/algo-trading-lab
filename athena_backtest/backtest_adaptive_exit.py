...
from configs_adaptive_exit import (
...
def build_snapshot(ts: pd.Timestamp, spot: float, vix: float,
                   ...
                   emer_strike: int = None, emer_entry: float = 0.0, emer_ltp: float = 0.0) -> dict:
...
def append_1min_snapshots_window(from_ts: pd.Timestamp, to_ts: pd.Timestamp,
    ...
    for ts, row in window.iterrows():
        ...
        # Exit checks
        # Adaptive Exit Logic:
        # Scenario 1: At 10:25, if VIX < 16, exit early to allow Artemis handoff.
        # Scenario 2: At 10:25, if VIX >= 16, continue until 15:10.
        if ts.hour == 10 and ts.minute == 25 and vix < VIX_HANDOFF_THRESHOLD:
            sl_hit_ts, sl_hit_reason = ts, 'early_exit'
            break
...
def run_backtest(nifty_1m: pd.DataFrame, vix_1m: pd.DataFrame,
    ...
    for expiry_idx, sell_expiry_date in enumerate(all_expiry_dates, 1):
        ...
        record = build_trade_record(
            ...
            emer_exit=emer_exit, emer_pl=window_realised_emer_pl, # this might be just the last window, but good enough for now
