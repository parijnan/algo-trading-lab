...
from configs_pe_parachute import (
    ...
    ENABLE_PE_EMERGENCY_HEDGE, PE_EMERGENCY_HEDGE_DELTA, PE_EMERGENCY_TRIGGER_OFFSET,
    PE_EMERGENCY_EXIT_OFFSET, PE_EMERGENCY_MAX_ATTEMPTS,
...
def select_strike(spot: float, sell_expiry: pd.Timestamp,
    ...
    logger.info(f"    [STRIKE-FAIL] No {option_type.upper()} strike found for {target_delta} delta at {entry_ts} (spot={spot:.1f})")
    return None, None
...
def build_snapshot(ts: pd.Timestamp, spot: float, vix: float,
                   ...
                   pe_wing_ltp: float = 0.0, pe_wing_entry: float = 0.0,
                   ce_wing_strike: int = None, pe_wing_strike: int = None,
                   emer_strike: int = None, emer_entry: float = 0.0, emer_ltp: float = 0.0,
                   pe_emer_strike: int = None, pe_emer_entry: float = 0.0, pe_emer_ltp: float = 0.0) -> dict:
    ...
    return {
        ...
        'emer_ltp':               round(emer_ltp, 2),
        'pe_emer_ltp':            round(pe_emer_ltp, 2),
...
def append_1min_snapshots_window(from_ts: pd.Timestamp, to_ts: pd.Timestamp,
    ...
    for ts, row in window.iterrows():
        ...
        if ENABLE_PE_EMERGENCY_HEDGE and buy_expiry_end is not None and opt_df_cache is not None:
            if not pe_emer_active and pe_emer_attempts < PE_EMERGENCY_MAX_ATTEMPTS:
                if spot <= pe_sell_strike + PE_EMERGENCY_TRIGGER_OFFSET:
                    stk, pr = select_strike(spot, buy_expiry_end, ts, 'pe', opt_df_cache, PE_EMERGENCY_HEDGE_DELTA)
                    if stk:
                        pe_emer_strike = stk
                        pe_emer_entry  = apply_slippage(pr, is_buy=True)
                        pe_emer_ltp    = pr
                        pe_emer_df     = opt_df_cache.get((buy_expiry_end, stk, 'pe'))
                        pe_emer_active = True
                        pe_emer_attempts += 1
            ...
            if pe_emer_active:
                ...
                if spot >= pe_sell_strike - PE_EMERGENCY_EXIT_OFFSET:
                    exit_pr = apply_slippage(pe_emer_ltp, is_buy=False)
                    realised_pe_emer = round(exit_pr - pe_emer_entry, 2)
                    window_realised_pl += realised_pe_emer
                    logger.info(f"  [EMERGENCY] Sold Parachute PE {pe_emer_strike} @ {exit_pr:.1f} at {ts} | P&L: {realised_pe_emer:.1f} | spot={spot:.0f}")
                    pe_emer_active = False
                    pe_emer_entry  = 0.0
                    pe_emer_ltp    = 0.0
...
def build_trade_record(entry_time, entry_spot, entry_vix,
                        ...
                        # Emergency Hedge fields
                        emer_strike=None, emer_entry=0.0,
                        emer_exit=0.0, emer_pl=0.0,
                        pe_emer_strike=None, pe_emer_entry=0.0,
                        pe_emer_exit=0.0, pe_emer_pl=0.0,
    ...
    # realized_pl includes locked-in gains from Emergency Hedge and previous adjustments
    # Note: emer_pl and pe_emer_pl are shown for informational purposes in the log,
    # but the actual P&L of closed hedge legs is captured in 'realised_pl'.
    # For the final summary, we sum everything.
    total_pl  = round(base_pl + adj_pl + realised_pl + emer_pl + pe_emer_pl, 2)
    ...
    return {
        ...
        'emer_entry':                  round(emer_entry, 2),
        'pe_emer_entry':               round(pe_emer_entry, 2),
        ...
        'emer_exit':                   round(emer_exit, 2),
        'pe_emer_exit':                round(pe_emer_exit, 2),
        'emer_pl':                     round(emer_pl, 2),
        'pe_emer_pl':                  round(pe_emer_pl, 2),
...
def run_backtest(nifty_1m: pd.DataFrame, vix_1m: pd.DataFrame,
    ...
    for expiry_idx, sell_expiry_date in enumerate(all_expiry_dates, 1):
        ...
        if adj_trigger_ts is not None and adj_winning_side is not None:
            ...
            if not otm_ok:
                ...
                (ce_sell_ltp, ce_buy_ltp, pe_sell_ltp, pe_buy_ltp,
                 ...
                 ce_wing_ltp, pe_wing_ltp,
                 window_realised_emer_pl,
                 emer_strike, emer_entry, emer_exit) = append_1min_snapshots_window(
        ...
        trade_log.append(build_snapshot(
            ...
            ce_wing_strike=ce_wing_strike, pe_wing_strike=pe_wing_strike,
            emer_strike=emer_strike, emer_entry=emer_entry, emer_ltp=0.0 # closed at exit
        ))
        ...
        record = build_trade_record(
            ...
            emer_exit=emer_exit, emer_pl=0.0, # informational
            pe_emer_strike=pe_emer_strike, pe_emer_entry=pe_emer_entry,
            pe_emer_exit=pe_emer_exit, pe_emer_pl=0.0, # informational
