...
(Trade counts above are closed trades only — each multiplier also has exactly 1 trade still open at data end.) Only mult 2.0 moved from the prior refresh (389→388 trades, ₹151,740→₹149,670) — the parity fix above blocked one entry (2026-05-01, an evening-only session) that only mult 2.0's own signal path happened to fire on; every other multiplier's raw signal sequence is byte-identical to before, as expected (the entry guard only matters on the rare evening-only sessions, and this is the only one across the whole grid where a multiplier's own flip timing landed inside the blocked window). `sweep_p3.py`'s own summary doesn't compute Calmar for the raw (no-SL/target) series — the qualitative finding stands regardless: raw Calmar climbed steadily from 5.5 down to 2.5, then flattened extending one step further to 2.0 rather than continuing to climb — the signature that argues against 2.5 being purely an under-explored grid-edge artifact.
...
**Checked against where the simulation already is.** *(Peak-units figures refreshed 2026-09-11 under the corrected simulator, up from the prior refresh — see "Backtest/production timing-guard parity fix" above — participation percentages rescaled linearly from the 2026-09-08 base run rather than re-pulled from raw volume data, which is exact for this purpose: participation_pct = order_size / that_minute's_volume × 100 is linear in order_size for any fixed minute, so every quantile of the distribution — median, p10, p5 — scales by the same ratio as the lot count itself. No approximation, just algebra.)* The dynamic-sizing runs above now reach peak units of 308 (no-slippage, up from 268) and 204 (slippage-adjusted, anchor coefficient, up from 181 — removing artifact-driven losses let both runs compound further before the slippage feedback loop caps growth) — 616 and 408 CRUDEOILM lots respectively (2 lots/unit), i.e. 6,160 bbl and 4,080 bbl of exposure:
...
Same question as the CRUDEOILM dynamic-sizing simulation earlier in this README, asked of the main contract: what if Prometheus had gone live on 2026-01-30 with `DYNAMIC_SIZING=True` on CRUDEOIL instead, using CRUDEOIL's own checked margin requirement — `MARGIN_PER_UNIT` = Rs 10,00,000 (10x CRUDEOILM's, tracking the contract's 10x lot size) and starting capital Rs 55,00,000, both user-supplied. Same live production combo (mult 2.0, SL 2.2%/T1 2.2%/T2 5.0% — T1 updated 2026-09-09, see Phase 3 caveat #1), run against CRUDEOIL's own 409-trade backtest (`phase3_crudeoil/data_sweep/mult_2.0/bespoke_trade_summary.csv`, refreshed 2026-09-11 under the corrected simulator — see "Backtest/production timing-guard parity fix" above — 20 more trades than CRUDEOILM's 389, same signal/exit logic against a different price series). Same per-trade equity/drawdown methodology (see "Drawdown methodology: per-trade, not per-lot-exit" above), and — from the start this time, not as a follow-up — both a no-slippage run and a slippage-adjusted run using the identical participation model from the CRUDEOIL liquidity comparison above (same A=0.3 anchor, carried over rather than re-fit, since a tick costs the same Rs/barrel on either contract).

**No-slippage result, refreshed 2026-09-11 under the corrected simulator**: 409 trades, Rs 55L → Rs 2.80Cr (+408.2%), max drawdown −18.2%, Calmar 22.37. Units grow from 5 to a peak of 30 (up from the prior refresh's 22, since removing artifact-driven losses lets capital compound further) — still a far more modest range than CRUDEOILM's 50→308, because CRUDEOIL's 10x-larger per-unit margin means the same rupee P&L moves units far less. Even so, an entry at peak size (30 units = 60 lots) runs ~125% median participation (linearly rescaled from the liquidity comparison's 83%-at-40-lots figure above) — well past the "1-2 tick" comfort zone, though nowhere near CRUDEOILM's equivalent-scaling extreme.

**Slippage-adjusted result, refreshed 2026-09-11 under the corrected simulator** (same feedback-loop mechanics as the CRUDEOILM slippage run — units resized from post-slippage capital every trade): final capital Rs 2.18Cr (+296.9%), max drawdown −19.2%, Calmar 15.49, peak units damped from 30 to 24 (the slippage feedback loop still caps growth well below the no-slippage peak). Coefficient sensitivity (0.5x/1x/2x anchor) holds the same ranking: Rs 2.53Cr → Rs 2.18Cr → Rs 1.69Cr final capital.

[Chart + table (both runs, comparison charts, sensitivity table)](https://claude.ai/code/artifact/704b21e1-1343-489b-8793-7d19240279ef) — structured identically to the CRUDEOILM artifact, rebuilt 2026-09-11 alongside this correction. Scripts: `phase3_crudeoil/dynamic_sizing_sim.py` and `phase3_crudeoil/dynamic_sizing_sim_slippage.py`, both committed. Detailed CSVs (`dynamic_sizing_trades.csv`, `dynamic_sizing_equity_curve.csv`, and their `_slippage` counterparts) in `phase3_crudeoil/data_sweep/mult_2.0/` (gitignored, run the scripts to regenerate).
...
**Refreshed 2026-09-11 under the corrected simulator** (389 trades — see "Backtest/production timing-guard parity fix" above, a bigger correction than the trailing-trade-drop fix that preceded it the same day; `phase3/risk_of_ruin_p3.py`, fixed seed 20260909 for reproducibility). Ruin remains vanishingly rare, and every supporting figure improved slightly (a cleaner, less artifact-contaminated edge):
...
- Of those 465, every single one recovered within the 2-year horizon — 32.9% within 1 month, 91.6% within 3 months, 99.8% within 6 months, 100% within a year. None qualified as a "long recovery."
- P(equity ever negative — literal wipeout): 0.00%.
- Max drawdown distribution: p50 16.8%, p90 29.2%, p95 34.3%, p99 47.1% — even the 99th-percentile bad-luck path is only borderline past the 40% mark, not blown through it.
- Median terminal equity after 2 years: ₹3.88 crore, from a ₹50,00,000 base — reflects the combo's edge (win rate 45.2%, avg win ₹3,354 vs. avg loss ₹1,805 per unit at 1-unit sizing), which is exactly why ruin is this rare in the simulation.
...
Same method (`phase3_crudeoil/risk_of_ruin_p3.py`), run against CRUDEOIL's own 409-trade `bespoke_trade_summary.csv`. **Sizing scale differs from CRUDEOILM's**: CRUDEOILM's 50 units came from a specific real decision ("user's current capital supports scaling to 50 lots," Position sizing section above); there's no equivalent documented decision for CRUDEOIL. This run uses 5 units (Rs 50,00,000 capital, `MARGIN_PER_UNIT`=Rs 10,00,000) — matching CRUDEOIL's own dynamic-sizing simulation's actual starting scale, not a stated capital-allocation decision. If a different CRUDEOIL scale is actually intended, this is a judgment call worth revisiting.

**Result: P(ruin) = 0.00%** (0 of 20,000 paths — back to zero after a single-path 0.01% reading at the previous, still-artifact-contaminated refresh; that one path was already flagged there as consistent with bootstrap noise on a low-probability tail event rather than a regime change, and this refresh confirms it):
- P(max drawdown > 40% at any point): 3.40% (679/20,000 paths) — still meaningfully higher than CRUDEOILM's 2.33%, consistent with CRUDEOIL's own lower Calmar (9.47 vs. CRUDEOILM's 14.76) and proportionally deeper drawdowns already found in the cross-validation above.
- Of those 679, all recovered within the 2-year horizon (100%, matching CRUDEOILM) — 28.4% within 1 month, 91.2% within 3 months, 99.7% within 6 months, 100% within a year.
- P(equity ever negative): 0.00%.
- Max drawdown distribution: p50 18.1%, p90 31.1%, p95 36.7%, p99 51.0% — the p95/p99 tail still sits meaningfully past the 40% ruin threshold, unlike CRUDEOILM's (29.2%/34.3%-region tail that stayed borderline). CRUDEOIL's ruin risk remains structurally higher than CRUDEOILM's at this sizing, not just a smaller version of CRUDEOILM's result.
- Median terminal equity: Rs 3.61 crore, from a Rs 50,00,000 base (vs. CRUDEOILM's Rs 3.88 crore at 10x the margin-per-unit — not a like-for-like capital-efficiency comparison, since the unit scales themselves were chosen differently, per the sizing-scale note above).
